...
#   FM_SPAWN_SECOND_MATE_RECOVERY=1 is reserved for recovery owners after they
#   prove a recorded secondmate dead or missing; only those paths may replace
#   retained kind=secondmate metadata during a relaunch.
...
if [ "$RELAUNCH" -eq 0 ] &&
	{ [ "$HOST_MODE" -eq 1 ] || [ "$KIND" = secondmate ]; } &&
	[ "$BACKEND" != orca ]; then
	SPAWN_ENDPOINT_CREATED=1
	SPAWN_ABORT_CLEANUP=1
fi
...
if [ "$HARNESS" = codex ] && [ "$KIND" != secondmate ]; then
	codex_notify_bash=$(type -P bash) || {
		echo "error: codex turn-end notification requires bash" >&2
		exit 1
	}
	case $(uname -s) in
...
if [ "$KIND" = secondmate ]; then
	...
	*) supervision_model=persistent ;;
	...
	# Deliver the primary's effective trace-context decision and clear inherited
	# host-root routing so a secondmate's own workers remain home-local.
	...
	MINGW* | MSYS* | CYGWIN*)
		...
		if [ -n "$SPAWN_TRACEPARENT" ]; then
			if spawn_record_traceparent; then
				trace_export="export TRACEPARENT=$(shell_quote "$SPAWN_TRACEPARENT")"
			else
				LAUNCH="unset TRACEPARENT; $LAUNCH"
			fi
		fi
		{
			...
			[ -z "$trace_export" ] || printf '%s\n' "$trace_export"
			printf '%s\n' "$LAUNCH"
		} >"$HERDR_WINDOWS_LAUNCH"
		chmod 600 "$HERDR_WINDOWS_LAUNCH"
		herdr_windows_bash=$(type -P bash) || {
			echo "error: native Windows Herdr launch requires bash" >&2
			exit 1
		}
