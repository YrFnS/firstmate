...
launch_template() {
  ...
  case "$harness" in
    ...
    # launch command in default mode - it is a Stop-event hook installed below
    # (global hook + per-task pointer). Host-root mode instead binds the process
    # to its task token because the endpoint cwd is the authoritative host root.
    grok)
      if [ "$HOST_MODE" -eq 1 ] && [ "$kind" != secondmate ]; then
