...
const repairOnlyHint =
	"call fm_watch_arm_pi again only after a later notification says the cycle is missing, failed, or unhealthy";
...
function actionableLine(output: string): string {
	...
	return (
		lines.find((line) =>
			/^(signal:|stale:|check:|heartbeat($|:))/.test(line),
		) || ""
	);
}
...
export default function (pi: ExtensionAPI) {
	...
	function deferredWakeIsQueued(wake: DeferredWake): boolean {
		...
		const keys = new Set(
			rows.map((fields) => `${fields[1]}\t${fields[2]}\t${fields[3]}`),
		);
		return wake.tokens.some((token) =>
			keys.has(`${token.seq}\t${token.kind}\t${token.key}`),
		);
	}
	...
	async function flushDeferredWake(owner: SessionGeneration): Promise<void> {
		...
		if (
			...
			wake.cycle !== owner.latestActionableCycle
		)
			return;
	...
	async function deliverActionableWake(
		...
		if (!generationIsLive(owner) || wake.cycle !== owner.latestActionableCycle)
			return;
		if (!busyAtClose) {
			await sendWake(owner, wake.message);
			return;
	...
	function scheduleRetry(
		...
		const timer = setTimeout(() => {
			...
			if (!result.ok) {
				surfaceFailure(
					owner,
					`watcher: FAILED - Pi extension could not launch a continuity retry\n${result.message}`,
				);
			}
	...
	function startArm(
		...
		if (ownership === "missing") {
			return {
				ok: false,
				message:
					"watcher: not armed - no live session holds the lock; run bin/fm-session-start.sh to reclaim it, then call fm_watch_arm_pi to re-arm",
			};
		}
		markLoaded();
		if (owner.child) {
		...
		armChild.on(
			...
			(code: number | null, signal: NodeJS.Signals | null) => {
				...
				if (classification.kind === "actionable") {
					...
					const tokens = busyAtClose
						? wakeQueueTokens(classification.message)
						: null;
					owner.latestActionableCycle = id;
					owner.retryFailures = 0;
					owner.restoring = true;
					void (async () => {
						const failure = await restoreAfterActionableClose(
							owner,
							predecessor,
						);
	...
	pi.registerCommand?.("fm-watch-arm-pi", {
		description:
			"Arm firstmate watcher supervision through the Pi extension instead of foreground bash.",
	...
	pi.registerTool?.({
		...
		description:
			"Start the first required Pi watcher cycle, or repair one only after a notification says the cycle is missing, failed, or unhealthy. Do not call after ordinary work or ordinary notifications; the Pi extension re-arms automatically. Never run bin/fm-watch-arm.sh through bash.",
		promptSnippet:
			"Start the first required Pi watcher cycle or repair a cycle reported missing, failed, or unhealthy; ordinary re-arming is automatic.",
		promptGuidelines: [
		...
		renderCall: (_args, theme, context) => {
			...
			if (calmPresentation.stockExportRendering) {
				return new Text(
					theme.fg("toolTitle", theme.bold("fm_watch_arm_pi")),
					0,
					0,
				);
			}
