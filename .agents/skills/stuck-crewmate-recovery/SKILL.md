...
For a host-root task, keep the recorded physical `host_root=` as the harness cwd and pass the existing recorded worktree separately as `FM_TARGET_WORKTREE`; [`docs/configuration.md`](../../../docs/configuration.md#host-root-mode-fm_host_root) owns that four-root exception.
