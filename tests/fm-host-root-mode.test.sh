...
test_session_cwd_mismatch_precedes_mutation() {
	...
	make_host "$host"
	mkdir -p "$home/state" "$home/data" "$home/config" "$other"
...
test_duplicate_spawn_preserves_existing_task() {
	...
	log="$TMP/duplicate.log"
	current="$TMP/duplicate.current"
	launch="$TMP/duplicate.launch"
	printf '%s\n' "$project" >"$current"
	: >"$current.endpoint"
...
test_spawn_rejects_host_as_target() {
	...
	log="$TMP/refusals.log"
	current="$TMP/refusals.current"
	tree_log="$TMP/treehouse.log"
...
test_spawn_rollback_is_transactional() {
	...
	current="$TMP/rollback.current"
	tree_log="$TMP/rollback-treehouse.log"
...
test_host_teardown_requires_confirmed_stop() {
	...
	make_host "$host"
	mkdir -p "$home/data/host-teardown" "$home/state" "$home/config"
	fm_git_init_commit "$project"
	git -C "$project" worktree add -q --detach "$wt"
	...
	log="$TMP/host-teardown.log"
	current="$TMP/host-teardown.current"
	tree_log="$TMP/host-teardown-treehouse.log"
	printf '%s\n' "$host" >"$current"
	: >"$current.endpoint"
	printf 'teardown-marker' >"$current.endpoint.marker"
	: >"$log"
	: >"$tree_log"
	out=$(cd "$host" && PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$home" FM_HOST_ROOT="$host" \
...
test_task_actions_use_recorded_host_root() {
	...
	log="$TMP/recorded-host.log"
	current="$TMP/recorded-host.current"
	: >"$log"
	printf '%s\n' "$host" >"$current"
	: >"$current.endpoint"
...
test_spawn_rejects_old_brief_and_secondmate_clears_roots() {
	...
	make_host "$host"
	mkdir -p "$home/data/old" "$home/state" "$home/config"
	...
	log="$TMP/secondmate.log"
	current="$TMP/secondmate.current"
	launch="$TMP/secondmate.launch"
	printf '%s\n' "$subhome" >"$current"
