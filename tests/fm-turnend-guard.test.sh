...
test_hook_non_claude_health_ignores_claude_budget_contention() {
	...
	while IFS='|' read -r harness payload; do
		out=$(printf '%s' "$payload" | FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" 2>&1)
		status=$?
...
test_hook_blocks_from_fm_home_state() {
	...
	out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" 2>&1)
	status=$?
...
test_hook_ignores_repo_state_when_fm_home_set() {
	...
	mkdir -p "$home/state"
	: >"$dir/state/task1.meta"
	out=$(printf '{"stop_hook_active":false}' | FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" 2>&1)
	status=$?
...
test_hook_uses_state_override() {
	...
	: >"$state/task1.meta"
	out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 FM_HOME="$home" FM_STATE_OVERRIDE="$state" bash "$dir/bin/fm-turnend-guard.sh" 2>&1)
	status=$?
...
test_hook_silent_without_stdin() {
	...
	: >"$dir/state/task1.meta"
	out=$(bash "$dir/bin/fm-turnend-guard.sh" </dev/null 2>&1)
	status=$?
...
EOF
	...
	out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
EOF
	chmod +x "$fakebin/grok"
	out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" GROK_TURNEND_GUARD_ACTIVE=1 bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
test_grok_adapter_native_false_blocks_without_resume() {
	...
	chmod +x "$fakebin/grok"
	out=$(printf '%s' '{"sessionId":"native","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
test_grok_adapter_native_true_allows_without_resume() {
	...
	printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" >"$fakebin/grok"
	chmod +x "$fakebin/grok"
	out=$(printf '%s' '{"sessionId":"native","stopHookActive":true}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
test_grok_adapter_snake_case_native_and_camel_precedence() {
	...
	: >"$dir/state/task1.meta"
	out=$(printf '%s' '{"sessionId":"native","stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
	...
	out=$(printf '%s' '{"sessionId":"native","stopHookActive":true,"stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
	expect_code 0 "$status" "camelCase true must win over snake_case false"
	out=$(printf '%s' '{"sessionId":"native","stopHookActive":false,"stop_hook_active":true}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
test_grok_adapter_invalid_inputs_start_neither_path() {
	...
	for payload in \
		...
		'{"sessionId":"x","sessionId":"y"}'; do
		out=$(printf '%s' "$payload" | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
		status=$?
	...
	out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$TMP_ROOT/missing-grok-root" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
SH
	...
	printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" >"$fakebin/grok"
	chmod +x "$fakebin/grok"
	out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
	...
	out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1)
	status=$?
...
EOF
	...
	out=$(
		NODE_NO_WARNINGS=1 PLUGIN="$plugin" DIRECTORY="$wrong_dir" WORKTREE="$worktree_dir" node 2>&1 <<'EOF'
...
SH
	...
	out=$(
		PLUGIN="$ext" FM_HOME="$home" FM_GUARD_LOG="$log" node --input-type=module 2>&1 <<'EOF'
...
SH
	chmod +x "$repo/bin/fm-turnend-guard.sh" "$repo/bin/fm-arm-pretool-check.sh"
	out=$(
		PLUGIN="$ext" FM_HOME="$home" node --input-type=module 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
test_hook_claude_mode_reblocks_stop_hook_active_when_unhealthy() {
	...
	: >"$dir/state/task1.meta"
	out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=200 run_hook_claude "$dir" true)
	status=$?
...
test_hook_claude_mode_allows_when_autoarm_owner_alive() {
	...
	out2=$(run_hook_claude "$dir" false)
	status2=$?
...
SH
	...
	guard_pid=$!
	IFS= read -r _ <"$ready"
	auto_out=$(run_integrated_autoarm "$dir")
	auto_status=$?
	printf 'release\n' >"$release"
	wait "$guard_pid"
...
test_hook_claude_mode_integrated_monotonic_fail_open() {
	...
	out=$(run_integrated_autoarm "$dir")
	status=$?
	...
	guard_out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true)
	guard_status=$?
	...
	guard_out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false)
	guard_status=$?
...
test_hook_claude_mode_concurrent_recovery_resets_are_idempotent() {
	...
	touch "$dir/state/.last-watcher-beat"
	(
		run_integrated_autoarm "$dir" >"$dir/auto.out"
		printf '%s\n' "$?" >"$dir/auto.status"
	) &
	auto_pid=$!
	(
		FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false >"$dir/guard.out"
		printf '%s\n' "$?" >"$dir/guard.status"
	) &
	guard_pid=$!
	wait "$auto_pid"
...
test_hook_claude_mode_budget_without_verified_failure_keeps_blocking() {
	...
	: >"$dir/state/task1.meta"
	for i in 1 2 3 4; do
		out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false)
		status=$?
...
test_hook_claude_mode_verified_failure_alarm_is_loud_and_once() {
	...
	seed_claude_failure "$dir"
	seed_claude_budget "$dir" 3
	out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true)
	status=$?
	...
	out2=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true)
	status2=$?
...
test_hook_claude_mode_fail_open_requires_notice_and_failure_epoch() {
	...
	out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$no_notice" true)
	status=$?
	...
	out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$notice_only" true)
	status=$?
...
test_hook_claude_mode_waits_for_late_claim() {
	...
	helper=$!
	out=$(FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=3000 run_hook_claude "$dir" false)
	status=$?
